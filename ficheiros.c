...
void read_file_countries(DADOS *dados, char *_nomeFilePaises, char *_nomeFileCidades)
{
    ...
    while (fgets(buffer, BUFFER_SIZE, fileInput) != NULL)
    {
        ...
        if (i % 100)
        {
            printf("\rProgresso: %d%%", ftell(fileInput) * 100 / totalFileSize);
        }
    }
...
void read_file_cities(DADOS *dados, char *_nomeFilePaises, char *_nomeFileCidades)
{
    ...
    while (fgets(buffer, BUFFER_SIZE, fileInput) != NULL)
    {
        ...
        //caso a linha acabe em \r remove-o
        if (buffer[bufferLen - 1] == '\r')
        {
            buffer[bufferLen - 1] == '\0';
        }

        a = malloc(sizeof(dados_temp));
        ...
        if (check == 11)
        {
            ...
            a->latitude.direcao = (strcmp(latitude_c, "N") == 0) ? NORTE : SUL;
            a->longitude.direcao = (strcmp(longitude_c, "E") == 0) ? ESTE : OESTE;
